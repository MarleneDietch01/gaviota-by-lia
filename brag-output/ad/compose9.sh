...
# puede reaparecer.
#
# EL CIERRE. El metraje se compone ENCIMA de base-9x16.mp4, y base.mp4 ya lleva el
...
# base-9x16.mp4 y el telefono se apagaria en negro en vez de fundirse a marfil.
...
ffmpeg -y -v error \
  -i base-9x16.mp4 \
  -i cut9_portada.mp4 -i cut9_catalogo.mp4 -i cut9_ficha.mp4 -i cut9_bolsa.mp4 \
  -i mask-9x16.png \
  -filter_complex "\
...
color=0xf7f0ec:s=486x1050:r=30:d=21,format=rgba[pc];[pc][m2]alphamerge[p];\
[0:v][p]overlay=297:518:enable='between(t,16,16.62)'[b1];\
[b1][af]overlay=297:518:enable='between(t,0,16.62)':eof_action=pass[v]" \
  -map "[v]" -map 0:a \
  -c:v libx264 -crf 17 -preset slow -pix_fmt yuv420p -r 30 \
  -c:a copy -movflags +faststart \
  ad9.mp4

ffprobe -v error -select_streams v:0 -show_entries stream=nb_frames,duration -of csv=p=0 ad9.mp4
