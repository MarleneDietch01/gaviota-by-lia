...
export default async function AdminOrderDetailPage({
  ...
  return (
    <div className="max-w-3xl">
      <Link href="/admin/orders" className="inline-flex items-center gap-1.5 text-sm font-medium text-body hover:text-gold-deep">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <span className="inline-flex rounded-pill bg-powder/40 px-2.5 py-1 text-xs font-semibold text-gold-ink">
      ...
      <section aria-labelledby="history-heading" className="mt-6 rounded-sm border border-line bg-white-warm p-5">
        ...
        {order.order_status_history?.length ? <ol className="mt-3 space-y-3">{[...order.order_status_history].sort((a,b)=>b.created_at.localeCompare(a.created_at)).map(entry=><li key={entry.id} className="border-l-2 border-gold-deep/40 pl-3 text-sm"><strong>{STATUS_LABEL[entry.new_status] ?? entry.new_status}</strong><p className="text-xs text-muted">{formatDate(entry.created_at)}{entry.note?` · ${entry.note}`:''}</p></li>)}</ol>:<p className="mt-2 text-sm text-muted">Sin cambios registrados.</p>}
      </section>
    </div>
  );
}
