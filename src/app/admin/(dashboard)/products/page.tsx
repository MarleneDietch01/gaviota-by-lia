  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Link href="/admin/products/new" className="min-h-10 rounded-xs bg-gold-deep px-5 py-2.5 text-sm font-semibold text-white-warm hover:bg-gold-ink">
          Crear producto
        </Link>
                  ...
                  return (
                    <tr key={product.id} className="border-b border-line last:border-0">
                      <td className="px-4 py-3">
                        <Link href={`/admin/products/${product.id}`} className="font-medium hover:text-gold-deep">
                          {product.name}
                        </Link>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="mb-2 flex justify-end gap-2">
                          <Link href={`/admin/products/${product.id}`} className="min-h-9 rounded-xs bg-gold-deep px-3 py-2 text-xs font-semibold text-white-warm">Editar</Link>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <Link
                  ...
                  className={`flex min-h-10 min-w-10 items-center justify-center rounded-xs border text-sm ${
                    p === page ? 'border-gold-deep bg-gold-deep/10 font-semibold text-gold-ink' : 'border-line text-body hover:border-ink'
                  }`}
                >
                  {p}
                </Link>
