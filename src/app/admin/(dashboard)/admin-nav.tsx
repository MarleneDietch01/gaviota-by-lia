      ...
      {NAV.map((item) => {
        ...
        return <li key={item.href} className={mobile ? 'contents' : undefined}>
          <Link href={item.href} aria-current={active ? 'page' : undefined}
            className={`flex min-h-10 shrink-0 items-center rounded-xs px-3 text-sm font-medium transition-colors ${active ? 'bg-ink text-white-warm' : 'text-ink hover:bg-ivory hover:text-gold-deep'}`}>
            {item.label}
          </Link>
        </li>;
      })}
