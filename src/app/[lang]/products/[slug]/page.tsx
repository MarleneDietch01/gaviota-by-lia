...
export default async function ProductPage({ params }: Props) {
  ...
  return (
    <div className="pb-24 lg:pb-0">
                ...
                {trustPoints.map((point) => {
                  ...
                  return (
                    <li key={point.title} className="flex gap-3 sm:px-4 sm:first:pl-0 sm:not-last:border-r sm:not-last:border-line">
                      <Icon className="mt-0.5 size-4 shrink-0 text-gold-deep" aria-hidden="true" />
                      <div>
          ...
          <Container>
            <div className="grid gap-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:gap-20">
              <div className="max-w-md">
                <p className="eyebrow mb-4 text-gold-ink">{pick(lang, 'The ritual', 'El ritual')}</p>
              ...
              <div className="divide-y divide-line-strong/60 rounded-sm bg-white-warm shadow-subtle">
                ...
                {product.precautions ? (
                  <details className="group p-6 sm:p-8">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-4 [&::-webkit-details-marker]:hidden">
                      <span className="eyebrow text-gold-deep">{pick(lang, 'Precautions', 'Precauciones')}</span>
                      <span aria-hidden="true" className="text-lg text-gold-deep transition-transform duration-200 group-open:rotate-45">+</span>
                    </summary>
      ...
      <Section tone="white">
        <Container>
          <p className="eyebrow text-gold-deep">{pick(lang, 'Your next step', 'Tu siguiente paso')}</p>
