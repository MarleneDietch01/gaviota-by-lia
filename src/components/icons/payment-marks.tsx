...
/**
 * Medios de pago que acepta el checkout.
 *
 * Los trazos son los oficiales de cada marca (simple-icons, MIT). Cada uno
 * lleva el `viewBox` recortado a la caja real del dibujo: el lienzo cuadrado
 * del original deja tanto aire vertical que un logotipo apaisado como VISA
 * quedaría diminuto al encajarlo en su casilla.
 *
 * Van a color sobre casillas crema, no en monocromo sobre el vino: como
 * silueta de un solo tono, Mastercard queda en un borrón y American Express en
 * un cuadrado negro — su marca oficial es la caja azul con el microtexto
 * "AMERICAN EXPRESS" calado, que a este tamaño solo se distingue por el color.
 *
 * Mastercard se dibuja aquí en vez de importarse: son dos círculos y su
 * intersección, geometría exacta, y así conserva los tres colores de la marca.
 *
 * HONESTIDAD: esta fila solo puede listar lo que el checkout acepta de verdad.
 * La sesión de Stripe se crea sin `payment_method_types`, así que los métodos
 * salen de la configuración del Dashboard. Verificado el 2026-09-08, en modo
 * real están habilitados: Tarjetas (todas las regiones — de ahí las cuatro
 * marcas de arriba, Discover incluida), Apple Pay, Google Pay, Link, Cash App
 * Pay, Amazon Pay, Affirm, Afterpay/Clearpay y Klarna.
 *
 * Aquí se muestran solo las tarjetas y las dos carteras universales: son las
 * que la clienta reconoce de un vistazo. Los seis restantes se aceptan igual
 * y podrían añadirse, pero doce logotipos en el pie se leen como un anuncio
 * de la pasarela, no como una señal de confianza. Los de "paga después"
 * (Klarna, Affirm, Afterpay) rinden junto al precio del producto —"4 pagos
 * de $10"— mucho más que como un logo suelto aquí abajo.
 *
 * Apple Pay y Google Pay aparecen según el navegador de la clienta: de ahí el
 * "donde estén disponibles" del resumen de la bolsa.
 */
const MARKS = {
  visa: {
...
function PathMark({ mark }: { mark: (typeof MARKS)[keyof typeof MARKS] }) {
  return (
    <svg
...
function MastercardMark() {
  return (
    ...
    >
      ...
      {/* La intersección real de los dos discos, recortando uno contra el
          otro: más fiable que calcular a mano el trazo de la lente. */}
      <circle cx="14" cy="12" r="12" fill="#ff5f00" clipPath="url(#mastercard-overlap)" />
    </svg>
  );
}

/** Casilla crema: el mismo gesto que el panel de marca del pie. */
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
      ...
      <ul className="flex flex-wrap items-center gap-2">
        ...
        <Tile label="American Express">
          <PathMark mark={MARKS.amex} />
        </Tile>
        {/* Casilla más ancha: el logotipo de Discover es solo la palabra, a
            casi 6:1, y en el ancho de las demás quedaría en 6 px de alto. */}
        <Tile label="Discover" wide>
          <PathMark mark={MARKS.discover} />
        </Tile>
        <Tile label="Apple Pay">
          <PathMark mark={MARKS.applePay} />
        </Tile>
        <Tile label="Google Pay">
          <PathMark mark={MARKS.googlePay} />
        </Tile>
      </ul>
    </div>
  );
