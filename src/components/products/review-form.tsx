  ...
  return (
    <form action={formAction} className="space-y-4 rounded-sm border border-line bg-white-warm p-5 sm:p-6">
      ...
      <button
        type="submit"
        disabled={pending}
        className="inline-flex min-h-12 w-full items-center justify-center rounded-xs bg-caramel text-sm font-semibold text-white-warm transition-colors duration-300 hover:bg-caramel-deep disabled:pointer-events-none disabled:opacity-60 sm:w-auto sm:px-8"
      >
        {pending
          ? locale === 'es'
            ? 'Enviando…'
